...
- Hardware is listed separately and hardware import is disabled by default. Enable experimental hardware import only after reviewing the source hardware diagnostics.
- Experimental camera hardware migration attempts to add source hardware to the first target recording server using the `AddHardware` task. If source credentials are not available, use the hardware fallback username/password fields.
